    ...
    <div my-6>
      <div>
        <div class="swiper">
          <div class="swiper-wrapper">
            <For each={roles()} fallback={<div>Inhale the moment. The lotus blooms in its own time……</div>}>
              {(item) => (
                <div
      ...
      <Show
        ...
        fallback={() => (
          <div class="h-12 my-4 flex gap-4 items-center justify-center bg-slate bg-op-15 rounded-sm">
            <span>Please maintain peace and joy...</span>
      ...
      >
        <div class="my-4 flex items-center gap-2 transition-opacity">
          <button title="Clear Conversation" onClick={clear} class="h-12 px-2 py-2 bg-slate bg-op-15 rounded-lg hover:bg-slate-50 transition-all duration-200">
            <IconClear />
          </button>
          <textarea
            ref={inputRef!}
            onKeyDown={handleKeydown}
            placeholder="Click on the avatar to start a conversation."
            autocomplete="off"
            onInput={() => {
              inputRef.style.height = 'auto';
          ...
          <button title="Export in Markdown format" onClick={exportToMarkdown} class="h-12 px-1 py-2 bg-op-15 hover:bg-op-20 transition-all duration-200">
